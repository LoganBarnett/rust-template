...
fn get_binary_path() -> PathBuf {
  ...
  path.push("rust-template-compliance-cli");
  ...
  match output {
    ...
    Err(e) => {
      if e.kind() == std::io::ErrorKind::NotFound {
        eprintln!(
                    "Binary not found. Please build the project first with: cargo build -p rust-template-compliance-cli"
                );
      }
...
fn test_version_flag() {
  ...
  match output {
    Ok(output) => {
      ...
      let stdout = String::from_utf8_lossy(&output.stdout);
      assert!(
        stdout.contains("rust-template-compliance-cli"),
        "Expected version text to contain 'rust-template-compliance-cli', got: {}",
        stdout
      );
    }
    Err(e) => {
