...
root. =docs/compliance.org= owns the authoritative process; the summary below
      shows which of the two paths applies to your repo.
...
from. Emit a fresh project from the current template into a temporary directory
      and compare against it file by file:
